...
- `local_server.py`: local-stable HTTPS runtime. It serves built assets, exposes `/healthz`, and handles native save/open/email bridge actions on macOS.
...
- Keep the local HTTPS server intentionally narrow: serve built assets and only the native endpoints required for macOS save/open/email workflows.
