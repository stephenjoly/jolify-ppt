...
function setBusy(isBusy: boolean) {
  ...
  if (runButton) {
    ...
    runButton.textContent = isBusy ? "Working..." : "Create Deck";
  }
...
async function runExport() {
  setBusy(true);
  setStatus("info", "Working", "Creating a new deck from the selected slides...");
  Office.context.ui.messageParent(JSON.stringify({ type: "run" }));
}
