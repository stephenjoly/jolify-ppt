...
export async function applySlideNumbersAcrossDeck(): Promise<ActionResult> {
  return PowerPoint.run(async (context) => {
    const slides = context.presentation.slides;
    slides.load("items");
    await context.sync();
    ...
    return {
      type: "success",
      message:
        `Applied slide numbers to ${slides.items.length - 1} slide${slides.items.length - 1 !== 1 ? "s" : ""}` +
        ` (added ${addedCount}, updated ${updatedCount}).` +
        (removedFromTitle > 0 ? " Removed existing number(s) from the title slide." : ""),
    };
  });
}
