...
const JOLIFY_SLIDE_NUMBER_NAME = "__jolify_slide_number__";
const SLIDE_NUMBER_WIDTH = 28;
const SLIDE_NUMBER_HEIGHT = 16;
const SLIDE_NUMBER_RIGHT_MARGIN = 18;
const SLIDE_NUMBER_BOTTOM_MARGIN = 10;
const SLIDE_NUMBER_FONT_SIZE = 10;
const SLIDE_NUMBER_COLOR = "#7F7F7F";
...
async function removeJolifySlideNumbers(): Promise<ActionResult> {
  return PowerPoint.run(async (context) => {
    const slides = context.presentation.slides;
    slides.load("items");
    await context.sync();
    ...
    for (const slide of slides.items) {
      ...
      shapes.items
        .filter((shape) => shape.name === JOLIFY_SLIDE_NUMBER_NAME)
        .forEach((shape) => {
          shape.delete();
          removed += 1;
        });
    ...
    return {
      ...
      message:
        removed > 0
          ? `Removed Jolify slide numbers from ${removed} shape${removed !== 1 ? "s" : ""}.`
          : "No Jolify slide numbers were present.",
    };
  });
}

async function addOrUpdateJolifySlideNumbers(): Promise<ActionResult> {
  return PowerPoint.run(async (context) => {
    const slides = context.presentation.slides;
    slides.load("items");
    await context.sync();
    ...
    for (let index = 0; index < slides.items.length; index += 1) {
      ...
      const existingNumbers = shapes.items.filter((shape) => shape.name === JOLIFY_SLIDE_NUMBER_NAME);

      if (index === 0) {
        existingNumbers.forEach((shape) => {
          shape.delete();
          removedFromTitle += 1;
        });
        await context.sync();
      ...
      const slideNumberText = String(index + 1);
      const numberShape = existingNumbers[0] ?? slide.shapes.addTextBox(slideNumberText, {
        left: SLIDE_WIDTH - SLIDE_NUMBER_WIDTH - SLIDE_NUMBER_RIGHT_MARGIN,
        top: SLIDE_HEIGHT - SLIDE_NUMBER_HEIGHT - SLIDE_NUMBER_BOTTOM_MARGIN,
        width: SLIDE_NUMBER_WIDTH,
        height: SLIDE_NUMBER_HEIGHT,
      });

      numberShape.name = JOLIFY_SLIDE_NUMBER_NAME;
      numberShape.left = SLIDE_WIDTH - SLIDE_NUMBER_WIDTH - SLIDE_NUMBER_RIGHT_MARGIN;
      numberShape.top = SLIDE_HEIGHT - SLIDE_NUMBER_HEIGHT - SLIDE_NUMBER_BOTTOM_MARGIN;
      numberShape.width = SLIDE_NUMBER_WIDTH;
      numberShape.height = SLIDE_NUMBER_HEIGHT;
      numberShape.fill.clear();
      numberShape.lineFormat.visible = false;
      numberShape.textFrame.leftMargin = 0;
      numberShape.textFrame.rightMargin = 0;
      numberShape.textFrame.topMargin = 0;
      numberShape.textFrame.bottomMargin = 0;
      numberShape.textFrame.autoSizeSetting = PowerPoint.ShapeAutoSize.autoSizeNone;
      numberShape.textFrame.verticalAlignment = PowerPoint.TextVerticalAlignment.bottom;
      numberShape.textFrame.wordWrap = false;
      numberShape.textFrame.textRange.text = slideNumberText;
      numberShape.textFrame.textRange.font.size = SLIDE_NUMBER_FONT_SIZE;
      numberShape.textFrame.textRange.font.color = SLIDE_NUMBER_COLOR;
      numberShape.textFrame.textRange.paragraphFormat.horizontalAlignment =
        PowerPoint.ParagraphHorizontalAlignment.right;

      existingNumbers.slice(1).forEach((shape) => {
        shape.delete();
      });
    ...
    return {
      type: "success",
      message:
        `Turned on Jolify slide numbers for ${slides.items.length - 1} slide${slides.items.length - 1 !== 1 ? "s" : ""}` +
        ` (added ${addedCount}, updated ${updatedCount}).` +
        (removedFromTitle > 0 ? " Removed existing Jolify number(s) from the title slide." : ""),
    };
  });
}

export async function toggleSlideNumbersAcrossDeck(): Promise<ActionResult> {
  return PowerPoint.run(async (context) => {
    const slides = context.presentation.slides;
    slides.load("items");
    await context.sync();
    ...
    let nonTitleSlides = 0;
    let slidesWithJolifyNumber = 0;
    ...
    for (let index = 0; index < slides.items.length; index += 1) {
      ...
      if (shapes.items.some((shape) => shape.name === JOLIFY_SLIDE_NUMBER_NAME)) {
        slidesWithJolifyNumber += 1;
      }
    }

    const shouldTurnOff = nonTitleSlides > 0 && slidesWithJolifyNumber === nonTitleSlides;
    return shouldTurnOff ? removeJolifySlideNumbers() : addOrUpdateJolifySlideNumbers();
  });
}
